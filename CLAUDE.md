...
- コメントは**日本語**で書く
- クラスとメソッドには `///` による XML ドキュメントコメントを記載する（`.razor` の `@code` ブロック内も含む）
  - `<summary>` は必須、補足が必要な場合のみ `<remarks>` を追加する
  - 引数がある場合は `<param>` を、戻り値がある場合は `<returns>` を必ず記載する
- `if` / `for` などの分岐・ループには `//` でその意図を説明するコメントを記載する
